...
%60rpm
function ac_dc_current_analysis_1()
    % clear; clc; 
    % close all;
...
%**************************記得確認彈簧常數、齒輪比、電阻***************************
    ...
    avg_cur_rev_results = NaN(num_voltages, 1);
    ...
    for i = 1:num_voltages
        ...
        % *** 新增：雙條件卡死檢測邏輯 ***
        % ------------------------------------------------------------------
        stall_speed_threshold = 1e-3; % rad/s
        stall_current_std_threshold = 1e-4; % A, 電流標準差閾值
        ...
        % 如果未卡死，則繼續進行週期分析
        revolutions = floor(x(segment_indices, 1) / (2*pi*params.n_gear));
        ...
        avg_cur_rev = mean(I_multi_cycle);
        avg_cur_rev_results(i) = avg_cur_rev;

        I_ac_parts = I_multi_cycle(I_multi_cycle > avg_cur_rev);
        if ~isempty(I_ac_parts)
            I_ac_results(i) = mean(I_ac_parts);
        else
            I_ac_results(i) = avg_cur_rev_results(i);
        end
        I_dc_parts = I_multi_cycle(I_multi_cycle<=avg_cur_rev);
        if isempty(I_dc_parts)
            I_dc_results(i) = avg_cur_rev_results(i);
        else
            I_dc_results(i) = mean(I_dc_parts);
        end
